...
export function GameProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (state.phase === 'rolling') {
      aiTimeoutRef.current = setTimeout(() => {
        setState(prev => {
          if (!prev || prev.currentPlayer !== 'black' || prev.phase !== 'rolling') return prev;
          const dice = rollDice();
    ...
    if (state.phase === 'moving') {
      aiTimeoutRef.current = setTimeout(() => {
        setState(prev => {
          if (!prev || prev.currentPlayer !== 'black' || prev.phase !== 'moving') return prev;
  ...
  }, [state?.currentPlayer, state?.phase, state?.remainingDice?.length, state?.mode]);
  ...
  const doRollDice = useCallback(() => {
    setState(prev => {
      ...
      if (prev.mode === 'vs-computer' && prev.currentPlayer === 'black') return prev;
      const dice = rollDice();
      return applyDiceRoll(prev, dice);
    });
  }, []);
  ...
  return (
    <GameContext.Provider value={{ state, startGame, resetGame, doRollDice, selectPoint, doMove }}>
      {children}
